$(function () {
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    ...
    var tabsHTML = '<ul class="tabs">'
        + '<li><a href="#managehome">Home</a></li>'
        + '<li><a href="#profile">Profile</a></li>'
        + '</ul>'

    $('<ul><li id="managehome"/><li id="profile"/></ul>').appendTo('#qunit-fixture')

    $(tabsHTML).find('li:last a').bootstrapTab('show')
    assert.strictEqual($('#qunit-fixture').find('.active').attr('id'), 'profile')

    $(tabsHTML).find('li:first a').bootstrapTab('show')
    assert.strictEqual($('#qunit-fixture').find('.active').attr('id'), 'managehome')
  })
  ...
  QUnit.test('should activate element by tab id in ordered list', function (assert) {
    ...
    $('<ol><li id="managehome"/><li id="profile"/></ol>').appendTo('#qunit-fixture')
  ...
  QUnit.test('should activate element by tab id in nav list', function (assert) {
    ...
    var tabsHTML =  '<nav class="nav">' +
                      '<a href="#managehome">Home</a>' +
                      '<a href="#profile">Profile</a>' +
                    '</nav>'

    $('<nav><div id="managehome"></div><div id="profile"></div></nav>').appendTo('#qunit-fixture')
  ...
  QUnit.test('should not fire shown when tab is already active', function (assert) {
    ...
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">'
      + '<li class="nav-item"><a href="#managehome" class="nav-link active" role="tab">Home</a></li>'
      + '<li class="nav-item"><a href="#profile" class="nav-link" role="tab">Profile</a></li>'
      + '</ul>'
      + '<div class="tab-content">'
      + '<div class="tab-pane active" id="managehome" role="tabpanel"></div>'
  ...
  QUnit.test('selected tab should have aria-expanded', function (assert) {
    ...
    var tabsHTML = '<ul class="nav nav-tabs">'
        + '<li><a class="nav-item active" href="#managehome" toggle="tab" aria-expanded="true">Home</a></li>'
  ...
  QUnit.test('selected tab should deactivate previous selected tab', function (assert) {
    assert.expect(2)
    var tabsHTML = '<ul class="nav nav-tabs">'
        + '<li class="nav-item"><a class="nav-link active" href="#managehome" data-toggle="tab">Home</a></li>'
  ...
  QUnit.test('selected tab should deactivate previous selected link in dropdown', function (assert) {
    assert.expect(3)
    var tabsHTML = '<ul class="nav nav-tabs">'
        + '<li class="nav-item"><a class="nav-link" href="#managehome" data-toggle="tab">Home</a></li>'
        + '<li class="nav-item"><a class="nav-link" href="#profile" data-toggle="tab">Profile</a></li>'
